					...
					return async ({ result }) => {
						...
						if (result.type === 'success') {
							...
							// toasts.error(result.data?.followError);
                             const errorMessage = typeof result.data?.followError === 'string' 
                                ? result.data.followError 
...
>
	...
	<div class="playlist-actions">
		{#if data.user?.id === playlist.owner.id}
			<Button element="a" href="/playlist/{playlist.id}/edit" variant="outline">Edit Playlist</Button>
		{:else if isFollowing !== null}
			<form
				class="follow-form"
				method="POST"
				...
				use:enhance={() => {
					...
					return async ({ result }) => {
						...
						if (result.type === 'success') {
							await applyAction(result);
							isFollowing = !isFollowing;
							toasts.success("Bonjour test de toast");
						} else if (result.type === 'failure') {
							//toasts.error(result.data?.followError);// valeur de type unkow il faut checker
							toasts.error(typeof result.data?.followError === 'string' ? result.data.followError : "Une erreur est survenue");
